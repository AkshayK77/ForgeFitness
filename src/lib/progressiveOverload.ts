...
export async function getProgressionSuggestion(userId: string, exerciseId: string): Promise<{
  ...
  type SetRow = { session_id: string; weight_kg: number | null; reps: number | null; created_at: string | null }
  ...
  const [exRes, setsRes] = await Promise.all([
    ...
    supabase.from('session_sets').select('session_id, weight_kg, reps, created_at').eq('exercise_id', exerciseId).eq('completed', true).order('created_at', { ascending: false }).limit(50),
  ])
