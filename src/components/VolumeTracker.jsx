...
const STATUS_BAR_COLORS = {
  none:    'rgba(85,85,85,0.3)',
  low:     'rgba(245,166,35,0.5)',
  optimal: 'rgba(200,245,90,0.6)',
  high:    'rgba(255,92,92,0.55)',
}

const ALL_MUSCLE_GROUPS = Object.keys(VOLUME_THRESHOLDS)

export default function VolumeTracker({ userId }) {
  ...
  if (loading) {
    return <p style={{ color: 'var(--muted)', fontSize: '13px' }}>Loading volume data…</p>
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {ALL_MUSCLE_GROUPS.map(mg => {
        const sets = volumeMap[mg]?.total_sets || 0
        ...
        return (
          <div key={mg} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {/* Name */}
            <div style={{ width: '110px', flexShrink: 0, fontSize: '13px', textTransform: 'capitalize' }}>
              {mg.replace(/_/g, ' ')}
            </div>
            ...
            <div style={{ flex: 1, height: '4px', background: 'var(--surface3)', borderRadius: '2px', position: 'relative', overflow: 'hidden' }}>
              {/* optimal zone marker */}
              <div style={{
                position: 'absolute', left: `${minPct * 100}%`, right: 0,
            ...
            {/* Sets / range */}
            <div style={{ width: '80px', textAlign: 'right', fontSize: '11px', color: 'var(--muted)', flexShrink: 0 }}>
