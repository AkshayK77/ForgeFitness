...
function MuscleCard({ muscle, defaultOpen, onExerciseClick }: { muscle: MuscleData; defaultOpen?: boolean; onExerciseClick: (ex: string) => void }) {
  const [open, setOpen] = useState(defaultOpen || false)
  const bColor = (BADGE_COLORS as Record<string, { bg: string; color: string; border: string }>)[muscle.badge] || BADGE_COLORS['Stabiliser']
...
export default function AnatomyPage() {
  ...
  const muscles = (DATA as Record<string, MuscleData[]>)[activePart] || []
      ...
      {isMobile ? (
        <div style={s.mobileTabStrip}>
          {allParts.map(part => (
            ...
            >
              {(PART_LABELS as Record<string, string>)[part]}
            </button>
          ))}
        </div>
      ) : (
      ...
      <main style={{ ...s.content, padding: isMobile ? '16px' : '24px 28px' }}>
        <div style={s.contentHeader}>
          <h1 style={{ ...s.pageTitle, fontSize: isMobile ? '24px' : '28px' }}>
            {(PART_LABELS as Record<string, string>)[activePart]}
          </h1>
